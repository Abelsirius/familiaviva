...
export default function Timeline({
    ...
    return (
        ...
        >
            ...
            >
                <div className="absolute top-0 right-0 w-24 h-24 bg-sky-100 rounded-bl-full opacity-50 z-0"></div>
                <div className="relative z-10">
                    <h3 className="font-bold text-sky-800 mb-4 flex items-center gap-2 text-lg">
                        <Clock size={24} className="text-sky-500" />
                        <span>Hito Personalizado</span>
                        <span className="text-xs font-normal bg-sky-100 text-sky-600 px-2 py-1 rounded-full">{babyAge}</span>
                    </h3>
...
function TimelineItem({ age, title, desc, color, icon, delay }) {
    return (
        <motion.div
            ...
            viewport={{ once: true }}
            transition={{ delay }}
            className={`p-5 rounded-2xl border ${color.replace('bg-', 'bg-opacity-40 bg-')} flex gap-4 shadow-sm hover:shadow-md transition-shadow`}
        >
            <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${color} bg-white`}>
                {icon}
            </div>
            <div>
                <span className="text-xs font-bold uppercase opacity-70 tracking-wide block mb-1">{age}</span>
